...
test('direct execution persists validated page-assets evidence for the exact original',async()=>{
 ...
 assert.match(instruction,/预算超限后父进程可能立即中断.*不能声称随后的 cleanup 调用已运行/);assert.match(instruction,/若 submission-intent 已持久化，结果按未知保护并禁止重发/);assert.doesNotMatch(instruction,/预算超限.*第二次 click 未执行/);
