...
6. 页面显示生成完成后，必须从当前这条最新 assistant 生成结果图片本身取得原始 PNG/JPG/WebP。不要只点击“保存”并等待 waitForEvent("download")：ChatGPT 的媒体保存按钮可能不产生浏览器 download 事件。先从当前这条结果的图片查看器读取可见 img 的 src，确认它不是参考附件缩略图，并从 URL 提取稳定的 OpenAI file id（匹配 file_[A-Za-z0-9_-]+）；不得用任意最新图片或模糊尺寸选择结果。然后使用当前公开的页面资产能力取得原文件：
   ...
   - 先在 inventory.assets 中筛选候选：kind === "image"，url 或 sourceUrl 是 backend-api/estuary/content，contentType 是 image/png、image/jpeg 或 image/webp，isThumbnail/isPreview 不为 true，role/name/url 不含 thumbnail、preview、缩略或预览；候选必须含有与当前 DOM src 相同的稳定 file id。若 URL 完全相同，matchingStrategy="exact-src"；若签名查询参数不同但稳定 file id 相同，matchingStrategy="stable-file-id"。二者都必须唯一，0 或大于 1 都拒绝。inventory 中 kind="other" 的同 file id 记录只能证明页面观察到了该文件，不能作为图片资产 bundle。
