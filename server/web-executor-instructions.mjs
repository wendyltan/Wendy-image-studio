...
- 唯一允许的创建调用是第一次 CUA js 调用中的 globalThis.__wendiOwnedTab=await cua.createBrowserTab("chrome",undefined,{sessionName:"🎨 温蒂生图"})；先创建一个自己拥有的空白专用 tab，再导航到 chatgpt.com。只创建一个专用 tab，不允许省略 sessionName、不允许再创建第二个标签页。创建调用返回后，立即把 globalThis.__wendiOwnedTabId=globalThis.__wendiOwnedTab.id||null 放入同一持久 CUA kernel，并输出当前 ownedTabId；后续每次 CUA 调用都必须从 globalThis.__wendiOwnedTab 取回句柄，不能用跨调用不会保留的 block-scoped const tab 代替。若 globalThis.__wendiOwnedTab 不存在，禁止调用 cua.getTab、cua.listTabs 或再次 createBrowserTab，立即执行 ${commands.browserHandleLost}，其中 ownedTabId 只填实际观察到的句柄或 unknown，owned-tab-cleanup-status 填 not_observed，kernel-reset 按事实填写；这表示句柄未知，不表示标签页已关闭。
- 若 createBrowserTab 或 tab.goto 出现 user declined、denied permission、browser security policy 等 chatgpt.com 站点源权限拒绝，必须执行 ${commands.originPermissionDenied}；这不是 BROWSER_CHROME_UNAVAILABLE，禁止上传或发送，也不要退回 IAB 或其他标签页重试。只有真正的 Chrome extension/浏览器能力不可用才记录 BROWSER_FOCUS_UNAVAILABLE 或 BROWSER_CHROME_UNAVAILABLE。
...
- 每次上传前先读取当前 DOM，不得依赖某一个固定中文按钮文案：从当前可访问名称中有限地寻找附件入口（例如“添加文件”“添加照片和文件”“上传文件”“附件”或英文 attach/upload 的按钮/菜单项，允许同义变体和正则匹配），确认它属于当前对话后再点击。点击后先用一个短时有界的 waitForEvent("filechooser") 观察是否直接打开选择器；创建 waiter 的同一表达式必须立刻附加 .catch(()=>null)，再 await 这个已处理的 promise，防止超时 rejection 在 CUA kernel 中变成未处理异常。若得到 null 或超时，只丢弃这个已结束的 waiter，重新读取 DOM，定位明确的菜单项（例如“从电脑上传”“上传文件”“添加照片和文件”或对应英文 upload/from computer），然后为这一次菜单点击创建全新的短时有界 filechooser waiter，同样立即附加 .catch(()=>null)，再 await chooser.setFiles(worker.referenceFiles)。每条分支只能执行一次实际上传动作，禁止保留悬挂 chooser promise、无限等待或用同一个 waiter 跨分支复用。先检查 chooser.isMultiple()：支持多选时按冻结顺序一次 setFiles 全部文件；不支持多选时，按附件顺序逐项重新读取 DOM、逐项点击入口/菜单、逐项 setFiles，并在每项后刷新 DOM 核对新增附件名称或数量。禁止调用 cua.getApp、macOS 原生文件选择器、系统鼠标或键盘。
- 上传后绝不能只因附件 group 已出现就发送。每轮从全新 DOM 逐个核对附件 group：group 总数必须等于 worker.referenceFiles.length，名称按冻结顺序匹配且不能重复或缺失。名称匹配允许 ChatGPT 为同一文件生成的确定性重复后缀：实际 basename 必须要么与冻结 basename 完全相同，要么只在原 basename 的扩展名前增加“(数字)”或“(YYYYMMDD-HHMMSS)”这一类 UI 去重后缀；去掉这一种后缀后必须与冻结 basename 逐字一致，禁止用模糊相似或任意改名通过。与此同时等待所有 group 的“上传中/正在上传/处理中/等待文件上传/uploading”状态消失。必须继续有界轮询（最长 240 秒），每轮写一条简短阶段日志（group 数量、规范化名称匹配、pending 状态、发送按钮 disabled 状态）。只有 group 数量与规范化名称全部匹配、没有任何等待/上传状态、发送按钮真实 enabled 且提示词仍在输入框时，才允许执行 ${commands.ready}、${commands.submissionIntent} 和一次发送。任何一个 group 卡住、名称/数量不匹配、状态仍为“等待文件上传”或发送按钮 disabled 到超时，都只执行 ${commands.uploadFailed}（若已记录 submission-intent 则执行 ${commands.confirmedUnsentUploadFailed}），绝不点击发送、绝不把 ready 当成上传成功、绝不重试第二次上传。
...
2. 在第一次 CUA js 调用中且只调用一次 globalThis.__wendiOwnedTab=await cua.createBrowserTab("chrome",undefined,{sessionName:"🎨 温蒂生图"})，并把返回句柄的 id 记录到 globalThis.__wendiOwnedTabId；不传入 visible，不创建第二个标签页。随后每一次独立 CUA 调用都先执行 const tab=globalThis.__wendiOwnedTab 并验证它仍然存在，再用这个 tab 执行 await tab.goto("https://chatgpt.com") 以及本次页面的导航、登录、上传、发送和下载。不要依赖跨调用的 const tab，因为它不会持久保留。逻辑上相当于 try { 使用 globalThis.__wendiOwnedTab } finally { 用同一句柄清理 }，但这些块不能跨 CUA 调用伪造；全部工作结束后，最后一次 CUA 调用才执行 await tab.close()；只有 close 返回成功才记录 cleanup-status=closed，异常记录 close_failed，句柄丢失记录 not_observed。创建时 Chrome 可能短暂取得焦点，公开 CUA 没有焦点恢复接口，因此不得声称零焦点切换或后台隐藏。${conversation}
