...
- CUA 输出必须保持紧凑：不要调用 getScreenshot()、getAXStateAndScreenshot()，不要把完整 domSnapshot()、页面截图、data:image base64 或带完整侧边栏的 DOM 原文通过 nodeRepl.write() 返回。需要读取 DOM 时，把快照只保存在当前脚本变量中，并只输出必要的布尔值、附件名称数组、发送按钮状态、当前 URL 和明确的错误摘要（总输出控制在 8KB 内）；后续动作必须依据这些精简字段继续。完整截图/base64 不属于状态证据，禁止写入事件、回复或下一轮上下文。
...
5. 发送前再次核对 provider 在本条指令中冻结的授权快照和 requestId；授权撤销则停止。点击前且只在一次已确认可用的发送按钮点击之前执行：
   ...
   只有 ok=true 后才能在紧接着的一个独立、最短 CUA 动作中点击一次；不要在 submission-intent 与点击之间重新上传、切换路由、重建提示词或输出完整 DOM/截图。点击后必须用新 DOM 正向证明至少一项：输入框已清空并出现本次新的用户消息，或页面已出现本次生成进度/停止生成控件。只有正向证据出现后，执行 ${commands.submitted}；只有 ok=true 才算 submitted。取得正向送达证据后执行 ${commands.ownedTabStage('sent','<实际 ownedTabId>')}；页面出现生成中/停止生成控件后执行 ${commands.ownedTabStage('generating','<实际 ownedTabId>')}。若点击返回但无法证明既未发送也未送达，执行 ${commands.submissionUncertain}；保留未知结果并绝不再点击。页面显示生成中时只等待，绝不再次发送。上传失败不得执行 sent 或 generating，界面必须保持“上传失败，未发送”。
