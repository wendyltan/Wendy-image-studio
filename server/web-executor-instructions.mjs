...
- 本地父流程在启动本执行器前已经持久、原子地完成唯一一次 reserve-create；浏览器执行器禁止再次调用 reserve-create、ensure 或任何其他 lease 创建命令。只有第一次 CUA js 调用才能执行 globalThis.__wendiOwnedTab=await cua.createBrowserTab("chrome",undefined,{sessionName:${JSON.stringify(resolvedSessionName)}})，且只能创建一个自己拥有的空白专用 tab，再导航到 chatgpt.com。createBrowserTab 返回后，立即把 globalThis.__wendiOwnedTabId=globalThis.__wendiOwnedTab.id||null 放入同一持久 CUA kernel，并用这个真实 id 执行 ${commands.ownedTabCreated('<实际 ownedTabId>','<本次 sessionName>')}；stage created 成功后才继续网页动作。不得省略 sessionName、不允许第二次 createBrowserTab。后续每次 CUA 调用都必须从 globalThis.__wendiOwnedTab 取回句柄，不能用跨调用不会保留的 block-scoped const tab 代替。若父流程未能完成 reserve、stage created 失败、globalThis.__wendiOwnedTab 不存在，或句柄调用抛出无法继续的错误，禁止调用 cua.getTab、cua.listTabs 或再次 createBrowserTab，立即执行 ${commands.browserHandleLost}，其中 ownedTabId 只填实际观察到的句柄或 unknown，owned-tab-cleanup-status 填 not_observed，kernel-reset 按事实填写；这表示句柄未知，不表示标签页已关闭。
