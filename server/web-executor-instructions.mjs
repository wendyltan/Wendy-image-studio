...
6. 页面显示生成完成后，必须从当前结果图片本身取得原始 PNG/JPG/WebP。不要只点击“保存”并等待 waitForEvent("download")：ChatGPT 的媒体保存按钮可能不产生浏览器 download 事件。先从当前这条最新生成结果的图片查看器读取可见 img 的 src，确认它不是参考附件缩略图；然后使用当前公开的页面资产能力取得原文件：
   ...
   - 在执行 ${commands.downloaded} 之前，使用本地文件写入能力把同一份机器证据 JSON 原子写入 ${JSON.stringify(path.resolve(downloadEvidenceFile))}。证据必须包含 schemaVersion=2、source="pageAssets"、projectId、projectVersion、taskId、requestId、runId、target、conversationUrl、capturedAt、currentResult.src（当前结果 img 的完整 src 和可选 resultId）、inventory.id、exactMatchCount=1、matchedAssetIds（只含一个 id）、matchedAsset（id/kind/contentType/url/sourceUrl/role/isThumbnail/isPreview）、bundle（downloadedCount=1、failures=[]、contentType/path）和 output（path/bytes/format/width/height/sha256）。这些身份字段必须逐字来自本次 worker-request.json/web-generation.json，不得自行填写其他任务值；currentResult.src 必须与 matchedAsset.url/sourceUrl 完全相同；kind 必须为 image，contentType 只能是 image/png、image/jpeg 或 image/webp，role 不能是 thumbnail/preview，output 必须对应刚复制的真实文件。匹配数为 0 或大于 1、身份字段与 request/worker/manifest/result 任一记录不一致、URL 不同、旧结果、缩略图/预览、非图片、文件不可解码或 hash 不一致时不得执行 downloaded，改执行已提交下载失败 helper，绝不重发。
