...
import {BROWSER_READINESS_EVALUATOR_SOURCE,BROWSER_READINESS_TIMEOUT_MS,BROWSER_READINESS_WAITER_SOURCE} from './browser-readiness.mjs';
...
export function chatGptWebImagePrompt({outputFile,manifestFile,prompt,remotePrompt=prompt,remotePromptLength=null,remotePromptSha256=null,referenceFiles=[],editTarget=null,conversationUrl=null,capsule='',requestId=null,runId=null,sessionName=null}){
  ...
  const readinessGate=`const waitForBrowserReadiness=${BROWSER_READINESS_WAITER_SOURCE}; let chatModeSwitchAttempted=false; const readiness=await waitForBrowserReadiness({timeoutMs:${BROWSER_READINESS_TIMEOUT_MS},pollIntervalMs:1000,read:async()=>{const currentUrl=await tab.url(); const ax=await tab.getAXState({emit:false,disableDiffing:true}); return evaluateReadiness({currentUrl,expectedUrl,ax,ownedTabId,runId});},onNotReady:async readiness=>{if(!chatModeSwitchAttempted && readiness?.checks?.targetUrlMatches===true && readiness.checks.loginRequired===false && readiness.checks.profileLoaded===true && readiness.checks.explicitChatMode===true && readiness.checks.chatModeEnabled===true && readiness.checks.chatModeSelected===false){chatModeSwitchAttempted=true; try{const chatMode=tab.playwright.getByRole('checkbox',{name:/^(聊天|Chat)$/}); if(await chatMode.count()===1 && await chatMode.isEnabled()) await chatMode.check({timeoutMs:5000});}catch{}}},wait:ms=>tab.playwright.waitForTimeout(ms)});`;
...
2. 父流程已经在启动执行器前持久完成 reserve-create；本步骤不得再次执行 reserve-create，也不得把 lease 从 creating 改回 not_created。第一次 CUA js 调用只执行一次 globalThis.__wendiOwnedTab=await cua.createBrowserTab("chrome",undefined,{sessionName:${JSON.stringify(resolvedSessionName)}})，并把返回句柄的真实 id 写入 globalThis.__wendiOwnedTabId；该 CUA 调用只创建并保留句柄，不混入本地 helper。随后以独立 command_execution 调用 ${commands.ownedTabCreated('<实际 ownedTabId>','<本次 sessionName>')}；实际 ID 必须来自返回句柄，禁止占位，且只有 helper 返回 ok=true 后才继续。不传入 visible，不创建第二个标签页。只有完成上述动作后，第二个业务 CUA 调用是唯一 bootstrap；调用 CUA 工具时必须在工具参数上显式设置 timeout_ms: 180000（毫秒），这是包含导航、就绪检查和结果返回的外层 180 秒绝对上限，不得依赖工具默认 timeout。导航耗时没有单独可控的 timeout；最多约 120 秒仅为导航/页面加载的预算预留，内部 readiness gate 仍最多 30000 毫秒，另预留约 30000 毫秒用于工具传输和返回。这些是预算分配，不保证导航能在预留时间内完成。若导航耗尽外层时限，CUA 调用可能整体超时并重置 kernel；此时可能遗留 orphan/cleanup_pending，不能保证能写入 typed graceful failure 或在原会话内完成 cleanup，必须保留未知/未确认状态，禁止重发。源码必须包含注释 // WENDI_BROWSER_READINESS_GATE_V1，从 globalThis.__wendiOwnedTab 取回并验证同一句柄，${conversation}，然后在同一个调用中执行 await tab.goto(${JSON.stringify(startUrl)}) 只调用一次 goto；禁止其他 goto/createBrowserTab/getTab。定义 evaluateReadiness 为 ${BROWSER_READINESS_EVALUATOR_SOURCE}，以 expectedUrl=${JSON.stringify(startUrl)}、本次真实 ownedTabId 和冻结 runId=${JSON.stringify(resolvedRunId)} 为参数，在最多 30000 毫秒内按以下结构执行：const deadline=Date.now()+30000; let readiness=null; while(Date.now()<deadline){const currentUrl=await tab.url(); const ax=await tab.getAXState({emit:false,disableDiffing:true}); readiness=evaluateReadiness({currentUrl,expectedUrl,ax,ownedTabId,runId}); if(readiness.ready) break; const remaining=deadline-Date.now(); if(remaining<=0) break; await tab.playwright.waitForTimeout(Math.min(1000,remaining));}。CUA 的 getAXState 默认可能只返回上一轮以来的变化片段；就绪检查每轮必须设置 disableDiffing:true 读取完整页面，并把返回字符串原样传给 evaluator。每轮只读取 URL/AX，不得操作网页、附件或发送。必须通过 nodeRepl.write("WENDI_BROWSER_READY_V1:"+JSON.stringify(readiness)) 输出单行证据。readiness.ready 为 true 的硬门条件是 target URL 精确匹配、无登录提示、profile 已加载、聊天模式明确选中且 enabled（若页面没有模式 checkbox，则仅接受已 active 的普通聊天 textbox）、composer 可用且 enabled、附件入口可见且 enabled；标准聊天 composer 路径即本任务可用的创建图片能力，不要求额外出现静态“创建图片”菜单文案。若 30 秒耗尽或任一条件无法证实，禁止进入附件、上传或发送；必须先写明确的提交前 typed failure：loginRequired 时执行 ${commands.loginFailed}，targetUrlMatches 不成立时执行 ${commands.navigationFailed}，其余 ui_not_ready 时执行 ${commands.browserModeEntryUnavailable}，再按第 8 步原句柄 cleanup。只有 readiness.ready === true 才能写入 uploading；缺少或未通过 readiness 证据时拒绝 uploading。紧接着以独立 command_execution 调用 ${commands.ownedTabStage('uploading','<实际 ownedTabId>')}；lease helper 必须从本 run events.jsonl 最新完整 CUA item.completed 读取并校验这次脚本产生的 WENDI_BROWSER_READY_V1 证据，否则拒绝 uploading。不得以 prompt、agent_message、command output 或较早事件替代，也不得忽略截断。只有上传阶段 helper 返回 ok=true 才能进入附件流程。后续每个 CUA 调用都只能取回同一自有句柄操作当前页面，不得再调用 goto、重建或切换会话。创建时 Chrome 可能短暂取得焦点；公开 CUA 没有焦点恢复接口，不得声称零焦点切换或后台隐藏。所有失败收尾只按第 8 步执行。
...
${frozenRemotePrompt.prompt}
  ...
  const bootstrapLoopPattern=/const deadline=Date\.now\(\)\+30000; let readiness=null; while\(Date\.now\(\)<deadline\)\{const currentUrl=await tab\.url\(\); const ax=await tab\.getAXState\(\{emit:false,disableDiffing:true\}\); readiness=evaluateReadiness\(\{currentUrl,expectedUrl,ax,ownedTabId,runId\}\); if\(readiness\.ready\) break; const remaining=deadline-Date\.now\(\); if\(remaining<=0\) break; await tab\.playwright\.waitForTimeout\(Math\.min\(1000,remaining\)\);\}/;
  if(!bootstrapLoopPattern.test(instruction))throw new Error('生成的 bootstrap readiness 循环不符合预期。');
  const guardedInstruction=instruction
    .replace(bootstrapLoopPattern,readinessGate)
    .replace('唯一 readiness bootstrap 必须在同一调用内最多等待 30 秒','唯一 readiness bootstrap 必须在同一调用内最多等待 60 秒')
    .replace('在最多 30000 毫秒内按以下结构执行','在最多 60000 毫秒内按以下结构执行')
    .replace('最多约 120 秒仅为导航/页面加载的预算预留，内部 readiness gate 仍最多 30000 毫秒，另预留约 30000 毫秒用于工具传输和返回。','约 90 秒仅为导航/页面加载的预算预留，内部 readiness gate 从 goto 返回后最多等待 60000 毫秒，另约 30 秒预留用于工具传输和返回。')
    .replace('若 30 秒耗尽','若 60 秒 readiness 窗口耗尽');
