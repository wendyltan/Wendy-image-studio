...
function publicProject(p){
  ...
  const publicPending=pending?{key:pending.key,at:pending.at,taskId:pending.taskId,provider:pending.provider||null,webState:webManifest?.state||null,requestId:webManifest?.requestId||null,accepted:webManifest?.accepted===true,acceptedAt:webManifest?.acceptedAt||null,readyAt:webManifest?.readyAt||null,submitted:webManifest?.submitted===true,submittedAt:webManifest?.submittedAt||null,downloadedAt:webManifest?.downloadedAt||null,errorCode:webManifest?.errorCode||null,ownedTabId:webManifest?.ownedTabId||null,sessionName:webManifest?.sessionName||null,ownedTabState:webManifest?.ownedTabState||null,cleanupStatus:webManifest?.cleanupStatus||webManifest?.ownedTabCleanupStatus||null,cleanupVerifiedAt:webManifest?.cleanupVerifiedAt||null,cleanupError:webManifest?.cleanupError||null,kernelReset:webManifest?.kernelReset===true}:null;
