...
function priorFailureFromManifest(manifest,execution=null){
  return {state:manifest?.state||null,errorCode:manifest?.errorCode||null,error:manifest?.error||null,failedAt:manifest?.failedAt||null,submissionUncertain:manifest?.submissionUncertain===true,failureStage:manifest?.failureStage||null,browserStage:manifest?.browserStage||null,runtimeErrorCategory:manifest?.runtimeErrorCategory||null,runtimeErrorMessage:manifest?.runtimeErrorMessage||null,runtimeErrorStack:manifest?.runtimeErrorStack||null,runtimeErrorToolStage:manifest?.runtimeErrorToolStage||null,runtimeErrorBrowserBudgetStage:manifest?.runtimeErrorBrowserBudgetStage||null,executorExecutionState:manifest?.executorExecutionState||execution?.state||null,artifactAcceptanceState:manifest?.artifactAcceptanceState||null,ownedTabState:manifest?.ownedTabState||null,ownedTabCleanupStatus:manifest?.ownedTabCleanupStatus||manifest?.cleanupStatus||null,cleanupError:manifest?.cleanupError||null};
}
...
export function createImageRecovery({webImageProvider,readGenerationEvidence,generationDiagnosticSummary,readWebManifest,pythonRun,jsonWrite,projectDir,inside,saveProject,job,verifyApproval,finishTask,activity,invalidatePanelDownstream,recordArtifact,attachImageRecord,artifactIdForImageKey,panelKeyFromImageKey,sampleIndexFromKey,sampleRepairCount,definiteImageFailure,failureMessage,hasActive=()=>false}={}){
  ...
  async function adoptNativeDownload(p,{dir=null,file=null,target=null,evidenceFile=null}={}){
    ...
    const recoveredAt=new Date().toISOString(),priorFailure=priorFailureFromManifest(manifest,run.execution);
    ...
    const evidence=checked.evidence,recoveredManifest=promoted.manifest,executorExecutionState=recoveredManifest.executorExecutionState||'failed',artifactAcceptanceState='recovered',recoveryNotice='原图已恢复，执行器曾异常。',timeline=recoveryTimeline({executionEndedAt:run.execution?.endedAt,priorResult:result,priorTask:task,downloadedAt:evidence.downloadedAt,recoveryCompletedAt:recoveredAt,now:Date.parse(recoveredAt)}),relativeFile=path.relative(projectRoot,expectedOutput);
    const record={key,file:relativeFile,provider:webImageProvider,conversationUrl:evidence.conversationUrl,prompt:pending.prompt,basePrompt:pending.basePrompt||pending.prompt,revisionDelta:pending.revisionDelta||null,revisionBase:pending.revisionBase||null,executor:{model:pending.executorModel||run.worker.executorModel||null,reasoningEffort:pending.executorReasoningEffort||run.worker.executorReasoningEffort||null,role:pending.role||run.worker.role||'browser-executor'},refs:Array.isArray(pending.refs)?pending.refs:[],telemetry:pending.telemetry||null,integrity,downloadEvidence:null,nativeDownloadEvidence:evidence,recoveredFrom:'native-download',executorExecutionState,artifactAcceptanceState,recoveryNotice,generationCompletedAt:timeline.generationCompletedAt,recoveryCompletedAt:timeline.recoveryCompletedAt,qa:{pass:null,status:'manual_review',summary:'原图已通过 Chrome 原生下载证据安全恢复，等待人工视觉校对。',issues:[],repairPrompt:'',source:'native_download_recovery'},at:recoveredAt};
    invalidatePanelDownstream(p,panelKey);attachImageRecord(p,record);recordArtifact(p,artifactIdForImageKey(key),'image',relativeFile,[],{integrity,nativeDownloadEvidence:evidence,recoveredFrom:'native-download',recoveryAt:recoveredAt,executorExecutionState,artifactAcceptanceState,recoveryNotice,priorFailure});jsonWrite(expectedOutput+'.json',record);
    writeRunResult(runDir,{schemaVersion:2,provider:webImageProvider,projectId:p.id,projectVersion:p.version,taskId:pending.taskId,target:key,requestId:recoveredManifest.requestId,runId:run.runId,endedAt:recoveredAt,generationCompletedAt:timeline.generationCompletedAt,priorCompletedAt:timeline.priorCompletedAt,recoveryCompletedAt:timeline.recoveryCompletedAt,submittedAt:recoveredManifest.submittedAt||evidence.submittedAt||null,downloadedAt:evidence.downloadedAt,acceptanceState:'downloaded',accepted:true,submitted:true,artifact:relativeFile,integrity,nativeDownloadEvidence:evidence,recoveredFrom:'native-download',sourceOutcome:result?.outcome||null,priorFailure,executorExecutionState,artifactAcceptanceState,recoveryNotice,diagnostics:generationDiagnosticSummary(runEvidence(runDir))});
    ...
    p.pending=null;p.lastFailure=null;p.error=null;p.status='paused';p.message=`${key} 已通过 Chrome 原生下载证据安全恢复，${recoveryNotice}等待人工视觉校对；未重新生图。`;p.recoveryImports=Array.isArray(p.recoveryImports)?p.recoveryImports:[];p.recoveryImports.push({sourceRunDir:runDir,sourceOutputFile:expectedOutput,importedAt:recoveredAt,target:key,projectId:p.id,projectVersion:p.version,taskId:pending.taskId,requestId:recoveredManifest.requestId,outputFile:expectedOutput,sha256:integrity.sha256,recoveredFrom:'native-download'});p.recoveryImports=p.recoveryImports.slice(-40);
    const webTimings={...task.webTimings,submittedAt:recoveredManifest.submittedAt||evidence.submittedAt||task.webTimings?.submittedAt||null,downloadedAt:evidence.downloadedAt,generationCompletedAt:timeline.generationCompletedAt,recoveryCompletedAt:timeline.recoveryCompletedAt,recoveredFrom:'native-download',executorExecutionState,artifactAcceptanceState,recoveryNotice,priorFailure};
    finishTask(p,task,'recovered_local',{completedAt:recoveredAt,generationCompletedAt:timeline.generationCompletedAt,priorCompletedAt:timeline.priorCompletedAt,recoveryCompletedAt:timeline.recoveryCompletedAt,artifact:expectedOutput,qa:'manual_review',qaStatus:'manual_review',webState:'downloaded',webTimings,errorCode:null,error:null,recoverySource:{runDir,outputFile:expectedOutput,requestId:recoveredManifest.requestId,downloadedAt:evidence.downloadedAt,sha256:integrity.sha256,recoveredFrom:'native-download'},nativeDownloadEvidence:evidence,executorExecutionState,artifactAcceptanceState,recoveryNotice,priorFailure});
    saveProject(p);
    return {project:p,runDir,file:expectedOutput,record,task,manifest:recoveredManifest,result, nativeDownloadEvidence:evidence,integrity,recoveredAt,executorExecutionState,artifactAcceptanceState,recoveryNotice,priorFailure};
  }
